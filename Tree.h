...
template <typename DataCenter,typename Key> class tree { // Key is a pointer to class key
private:
    ...
    public:
        ...
        vertex *right_son;
        int height;
        vertex(Key input_key, DataCenter input_data) : key(input_key) ,data(input_data),left_son(nullptr),right_son(nullptr),height(0) {};
        ...
        void swapVertex(vertex *ver1, vertex *ver2) {
            ...
            DataCenter temp_data = ver1->data;
            ver1->data = ver2->data;
            ver1->key = ver2->key;
            ver2->key = temp_key;
            ver2->data = temp_data;
    ...
    Vertex addVertexAux(Key input_key, DataCenter input_data, Vertex curr_root) {
        vertex ver(input_key, input_data);
        Vertex v = &ver;
        if (curr_root == nullptr) { // this means we've reached a leaf
            curr_root = new vertex(input_key, input_data);
            ...
            curr_root->left_son = addVertexAux(input_key, input_data,
                                               curr_root->left_son);
        } else if (*(v->key) > *(curr_root->key)) {
            curr_root->right_son = addVertexAux(input_key, input_data,
                                                curr_root->right_son);
        }
    ...
    Vertex removeVertexAux(Vertex v, Vertex curr_root, bool* is_deleted) {
        ...
        if (curr_root == nullptr)
            return nullptr;
        curr_root->height =
                max(height(curr_root->right_son), height(curr_root->left_son)) +
                1;
        curr_root = balanceTree(curr_root);
        return curr_root;
    };
...
public:
    ...
    void addVertex(Key key, DataCenter input_data) {
        root = addVertexAux(key, input_data, root);
    }
    ...
    DataCenter getDataByKey(Key key) {
        Vertex temp = findVertexByKey(key, root);
        return temp->data;
    }
    ...
    Key* reverseInorder(int server_num){
        int num = 0;
        Key* key_array = new Key[server_num];
        reverseInorderAux(root,key_array,&num);
        return key_array;
    }
    void reverseInorderAux(Vertex curr_root ,Key* keys_array, int* num){
        if(curr_root == nullptr){
            return;
        }
        reverseInorderAux(curr_root->right_son, keys_array, num);
        keys_array[*num] = curr_root->key;
        (*num)++;
        reverseInorderAux(curr_root->left_son, keys_array, num);
    }
    void reverseInorderGetData(Vertex curr_root ,DataCenter* data_array, int* num){
        if(curr_root == nullptr){
            return;
        }
        reverseInorderGetData(curr_root->right_son, data_array, num);
        data_array[*num] = curr_root->data;
        (*num)++;
        reverseInorderGetData(curr_root->left_son, data_array, num);
    }
    void countInorder(int* num, Vertex curr_root){
        if(curr_root == nullptr){
            return;
        }
        countInorder(num, curr_root->left_son);
        (*num)++;
        countInorder(num, curr_root->right_son);
    }
};
#endif //EX1_TREE_H